...
module.exports = {
     ...
     */
    // TODO: Kill in favor of Number.isInteger
    ...
    forEachChildElement(node, handler) {
        for (let i = 0; i < node.childNodes.length; i++) {
            const child = node.childNodes[i];
            if (this.getNodeType(child) !== "element") continue;
            handler(child, i);
        }
    },

    mapChildElements(node, handler) {
        const results = [];
        this.forEachChildElement(node, (child, i) => {
            const result = handler(child, i);
            results.push(result);
        });
    ...
    findElements(node, tagName, max) {
        ...
        for (let i = 0; i < node.childNodes.length; i++) {
            ...
            elements.push(child);
            if (max && elements.length >= max) break;
        }

        return elements;
    },

    findFirstElement(node, tagName) {
        return this.findElements(node, tagName, 1)[0];
    },

    findNthElement(node, n, tagName) {
        return this.findElements(node, tagName, n + 1)[n];
    },
    ...
    // TODO: Private (if exposing utils in Workbook)
    getRegExpForSearch(pattern) {
