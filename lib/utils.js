...
module.exports = {
    ...
    getNodeText: function (node) {
        for (var i = 0; i < node.childNodes.length; i++) {
            if (this.xmlDomNodeType(node.childNodes[i]) === 'Text') {
    ...
    getNodeInfo: function (node, buffer) {
        buffer = buffer || [];
        buffer.push('nodeName: ' + node.nodeName);
        buffer.push('nodeType: ' + this.xmlDomNodeType(node));
        buffer.push('nodeText: ' + this.getNodeText(node));
        buffer.push('nodeValue: ' + node.nodeValue);
        return buffer.join(', ');
    }
};
