...
class _Relationships {
    ...
    findByType(type) {
        ...
        return this._node.children.find(node => jq.get(node, "attributes.Type") === `${RELATIONSHIP_SCHEMA_PREFIX}${type}`);
    }
    ...
    _getStartingId() {
        ...
        this._node.children.forEach(node => {
            const id = parseInt(jq.apply(node, "attributes.Id", id => id.substr(3)));
