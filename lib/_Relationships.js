...
class _Relationships {
    ...
    constructor(node) {
        debug("constructor(_)");
        this._node = node;
        this._relationshipNode = node.Relationships.Relationship;
    }
    ...
    add(type, target) {
        ...
        // TODO: Kill this because it causes identical workbooks to be different each time. Follow the pattern from numFmts
        const id = Date.now();
        const node = {
            $: {
                Id: `rId${id}`,
        ...
        this._relationshipNode.push(node);
        return node;
    }
    ...
    findByType(type) {
        ...
        return this._relationshipNode.find(relationship => relationship.$.Type === `${RELATIONSHIP_SCHEMA_PREFIX}${type}`);
    }
