...
class Sheet {
    ...
    toXmls() {
        ...
        // Add the headerFooter if needed.
        this._headerFooterNode.children = _.values(this._headers).concat(_.values(this._footers));
        if (this._headerFooterNode.children.length) {
            xmlq.insertInOrder(node, this._headerFooterNode, nodeOrder);
        }
     ...
     * @param {string} attributeName - Attribute name of the headerFooterOptions.
     *   differentOddEven - Display a different header and footer for odd or even pages.
     *   differentFirst - Display a different header and footer for the first page.
     *   scaleWithDoc - The header and footer scales with document size.
     ...
     * @param {string} attributeName - Attribute name of the headerFooterOptions. See get header/footer option for list of valid attributes.
     * @param {undefined|boolean} attributeEnabled - If `undefined` the attribute is set to default, otherwise the header/footer option is set.
     * @returns {Sheet} The sheet.
     */
    headerFooterOptions() {
        const defaultValues = {
        ...
        const checkAttributeName = this._getCheckAttributeNameHelper('headerFooterOptions', supportedAttributeNames);
        return new ArgHandler('Sheet.headerFooterOptions')
            .case(['string'], attributeName => {
                checkAttributeName(attributeName);
                if (typeof this._headerFooterNode.attributes[attributeName] === 'undefined') {
                    return defaultValues[attributeName];
                }
                return this._headerFooterNode.attributes[attributeName] === 1;
            })
            .case(['string', 'nil'], attributeName => {
                checkAttributeName(attributeName);
                delete this._headerFooterNode.attributes[attributeName];
                return this;
            })
            .case(['string', 'boolean'], (attributeName, attributeEnabled) => {
                checkAttributeName(attributeName);
                if (defaultValues[attributeName] === attributeEnabled) {
                    this.headerFooterOptions(attributeName, null);
                } else {
                    this._headerFooterNode.attributes[attributeName] = attributeEnabled ? 1 : 0;
                }
                return this;
            })
            .handle(arguments);
     ...
     * @returns {string|undefined} The value of the header.
     *//**
     * Gets the value of the header for this sheet given a location of the header.
     * @param {string} locationName - The location of the header.
     *   odd - The header that appears on the odd pages only. If differentOddEven is disabled this will be ignored.
     *   even - The header that appears on the even pages only. If differentOddEven is disabled this will be ignored.
     *   first - The header that appears on the first page only. If differentFirst is disabled this will be ignored.
     *   default - The header that appears on every page if neither attributes are enabled.
     * @returns {string|undefined} The value of the header.
     *//**
     * Sets the value of the header for this sheet.
     * @param {string} locationName - The location of the header. See get header for list of valid locations.
     * @param {string|undefined} value - The value to set.
     * @returns {Sheet} This sheet.
     */
    header() {
        const supportedLocationNames = ['odd', 'even', 'first', 'default'];
        const checkLocationName = this._getCheckAttributeNameHelper('header', supportedLocationNames);
        return new ArgHandler('Sheet.header')
            .case([], () => {
                if (typeof this._headers.odd === 'undefined') {
                    return undefined;
                }
                return this._headers.odd.children[0];
            })
            .case(['string'], locationName => {
                checkLocationName(locationName);
                locationName = locationName === 'default' ? 'odd' : locationName;
                if (typeof this._headers[locationName] === 'undefined') {
                    return undefined;
                }
                return this._headers[locationName].children[0];
            })
            .case(['string', 'nil'], locationName => {
                checkLocationName(locationName);
                locationName = locationName === 'default' ? 'odd' : locationName;
                delete this._headers[locationName];
            })
            .case(['string', 'string'], (locationName, value) => {
                checkLocationName(locationName);
                locationName = locationName === 'default' ? 'odd' : locationName;                
                if (typeof this._headers[locationName] === 'undefined') {
                    this._headers[locationName] = { name: `${locationName}Header`, children: [] };
                }
                this._headers[locationName].children[0] = value;
                return this;
            })
            .handle(arguments);
    }
     ...
     * @returns {string|undefined} The value of the footer.
     *//**
     * Gets the value of the footer for this sheet given a location of the footer.
     * @param {string} locationName - The location of the footer.
     *   odd - The footer that appears on the odd pages only. If differentOddEven is disabled this will be ignored.
     *   even - The footer that appears on the even pages only. If differentOddEven is disabled this will be ignored.
     *   first - The footer that appears on the first page only. If differentFirst is disabled this will be ignored.
     *   default - The footer that appears on every page if neither attributes are enabled.
     * @returns {string|undefined} The value of the footer.
     *//**
     * Sets the value of the footer for this sheet.
     * @param {string} locationName - The location of the footer. See get footer for list of valid locations.
     * @param {string|undefined} value - The value to set.
     * @returns {Sheet} This sheet.
     */
    footer() {
        const supportedLocationNames = ['odd', 'even', 'first', 'default'];
        const checkLocationName = this._getCheckAttributeNameHelper('footer', supportedLocationNames);
        return new ArgHandler('Sheet.footer')
            .case([], () => {
                if (typeof this._footers.odd === 'undefined') {
                    return undefined;
                }
                return this._footers.odd.children[0];
            })
            .case(['string'], locationName => {
                checkLocationName(locationName);
                locationName = locationName === 'default' ? 'odd' : locationName;
                if (typeof this._footers[locationName] === 'undefined') {
                    return undefined;
                }
                return this._footers[locationName].children[0];
            })
            .case(['string', 'nil'], locationName => {
                checkLocationName(locationName);
                locationName = locationName === 'default' ? 'odd' : locationName;
                delete this._footers[locationName];
            })
            .case(['string', 'string'], (locationName, value) => {
                checkLocationName(locationName);
                locationName = locationName === 'default' ? 'odd' : locationName;                
                if (typeof this._footers[locationName] === 'undefined') {
                    this._footers[locationName] = { name: `${locationName}footer`, children: [] };
                }
                this._footers[locationName].children[0] = value;
                return this;
            })
            .handle(arguments);
    }
    ...
    _init(workbook, idNode, node, relationshipsNode) {
        ...
        // Create the headerFooter.
        this._headerFooterNode = xmlq.findChild(this._node, "headerFooter");
        if (this._headerFooterNode) {
            xmlq.removeChild(this._node, this._headerFooterNode);
        } else {
            this._headerFooterNode = { name: 'headerFooter', attributes: {}, children: [] };
        }

        const headerFooterNodes = this._headerFooterNode.children;
        this._headerFooterNode.children = [];
        headerFooterNodes.forEach(headerFooterNode => {
            const name = headerFooterNode.name;
            const headerOrFooter = name.substr(-6);
            const start = name.substr(0, name.length - 6);
            if (headerOrFooter === 'Header') {
                this._headers[start] = headerFooterNode;
            } else if (headerOrFooter === 'Footer') {
                this._footers[start] = headerFooterNode;
            }
        });
