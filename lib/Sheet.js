...
class Sheet {
    ...
    /**
     * Append page-breaks after the col/row.
     * @param {number} id - row|column number.
     * @param {boolean} isVertical - isVertical, default false.
     * @returns {undefined}
     * @private
     */
    _addPageBreak(id, isVertical) {
        const nodeName = isVertical ? 'colBreaks' : 'rowBreaks';
        let node = xmlq.findChild(this._node, nodeName);
        if (!node) {
            node = {
                name: nodeName,
                children: [],
            ...
            xmlq.insertInOrder(this._node, node, nodeOrder);
        }
        node.children.push({
            name: "brk",
            children: [],
        ...
        node.attributes.manualBreakCount++;
    }
}
