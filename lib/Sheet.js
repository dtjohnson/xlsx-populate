...
class Sheet {
     ...
     * @param {undefined|boolean} enabled - If `undefined` then the attribute is removed, otherwise set as a boolean value to enable/disable the print option.
     * @returns {Sheet} The sheet.
     * @ignore
     */
    printOptions() {
        ...
        return new ArgHandler('Sheet.printOption')
            ...
            .case(['string', 'boolean'], (attributeName, enabled) => {
                checkAttributeName(attributeName);
                this._printOptionsNode.attributes[attributeName] = enabled ? 1 : 0;
                return this;
            })
            .handle(arguments);
    }
    ...
    /**
     * Get a helper function to check that the attribute name provided is supported. If the attribute is not supported, an error is thrown.
     ...
     * @returns {function} The helper function, which takes a attribute name. If the array of supported attribute names does not contain the given attribute name, then an error is thrown.
     */
