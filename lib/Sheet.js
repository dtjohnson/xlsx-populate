...
class Sheet {
     ...
     * @param {string} address - The address to check.
     * @returns {object|undefined} Object or undefined if not set
     * @ignore
     *//**
     * Add dataValidation to cells at the given address if object is an object and not null, remove if null
     * @param {string} address - The address to merge.
     * @param {object} obj - Object to set, null to delete.
     * @returns {Sheet} The sheet.
     * @ignore
     */
    dataValidation() {
        return new ArgHandler('Sheet.dataValidation')
            .case('string', (address) => {
                if(this._dataValidations[address]) {
                    return {
                        name: this._dataValidations[address].name,
                        type: this._dataValidations[address].attributes.type,
                        allowBlank: (this._dataValidations[address].attributes.allowBlank ? true : false),
                        showInputMessage: (this._dataValidations[address].attributes.showInputMessage ? true : false),
                        prompt: this._dataValidations[address].attributes.prompt,
                        promptTitle: this._dataValidations[address].attributes.promptTitle,
                        showErrorMessage: (this._dataValidations[address].attributes.showErrorMessage ? true : false),
                        ...
                        address: this._dataValidations[address].attributes.sqref,
                        formula1: this._dataValidations[address].children[0].children[0],
                        formula2: this._dataValidations[address].children[1].children[0],
                    };
                } else {
                    return undefined;
                }
            ...
            .case(['string', '*'], (address, obj) => {
                if(obj !== null) {
                    ...
                    delete this._dataValidations[address];
                }
                return this;
            })
            .handle(arguments);
