...
class Sheet {
    ...
    //  */
    constructor(workbook, idNode, node, relationshipsNode, drawingsNode, drawingsReplationshipNode) {
        this._init(workbook, idNode, node, relationshipsNode, drawingsNode, drawingsReplationshipNode);
    }
     ...
     * @returns {Object.<string, { ref: string, formula: string }>} Dictionary of formulas
     *//**
     * Get a given shared formula.
     * @param {string} id - The shared formula id
     * @returns {Object.<{ ref: string, formula: string }>} The shared formula
     *//**
     * Set or update a given shared formula by its id
     * @param {string} id - The shared formula id
     * @param {Object.<{ ref: string, formula: string }>} sharedFormula - The shared formula
     * @returns {Sheet} The sheet.
     */ sharedFormulas() {
     ...
     * @returns {Row[]} Rows of the sheet.
     */ rows() {
        return new ArgHandler("Sheet.rows")
            .case(() => {
                return _.compact(this._rows);
            })
            .handle(arguments);
    }
    ...
    definedName() {
        return new ArgHandler("Workbook.definedName")
            .case(() => {
                return this.workbook().scopedDefinedNames(this);
            })
            .case('string', name => {
     ...
     * @returns {boolean} True if sheet has protection
     *//**
    * Set whether the sheet is password protected or not.
    * @param {string} password - Value to protect sheet modifications.
    * @returns {Sheet} The sheet.
    */
    protected() {
        return new ArgHandler('Sheet.protected')
            .case(() => {
                if (this._sheetProtectionNode) return true;
                return false;
            })
            .case(['string', "*"], (password, options) => {
                this._sheetProtectionNode = null;
                ...
                if (encryptor && password) {
                    ...
                    const permissions = options ? {
                        objects: options.objects ? "1" : "0",
                        scenarios: options.scenarios ? "1" : "0",
                        selectLockedCells: options.selectLockedCells ? "1" : "0",
                        selectUnlockedCells: options.selectUnlockedCells ? "1" : "0",
                        formatCells: options.formatCells ? "1" : "0",
                        formatColumns: options.formatColumns ? "1" : "0",
                        formatRows: options.formatRows ? "1" : "0",
                        insertColumns: options.insertColumns ? "1" : "0",
                        insertRows: options.insertRows ? "1" : "0",
                        insertHyperlinks: options.insertHyperlinks ? "1" : "0",
                        deleteColumns: options.deleteColumns ? "1" : "0",
                        deleteRows: options.deleteRows ? "1" : "0",
                        sort: options.sort ? "1" : "0",
                        autoFilter: options.autoFilter ? "1" : "0",
                        pivotTables: options.pivotTables ? "1" : "0"
                    } : {}

                    this._sheetProtectionNode = {
     ...
     * @returns {Range[]} Ranges of merged cells of the sheet.
     *//**
    ...
    merged() {
        return new ArgHandler('Sheet.merge')
            .case(() => {
                return _.keys(this._mergeCells).map(key => {
                    const address = key.includes(":") ? key : `${key}:${key}`;
    ...
    /**
	 * Get all Drawings on sheet
	 * @returns {array} of Drawing Objects
	 * @ignore
	*//**
	 * Gets a Drawing Object or unidentified if no drawing name is found
	 * @param {string} nane - defined name for the image.
	 * @returns {Drawing | undefined} Drawing Class for that image or undefined if its not there.
	 * @ignore
	 */
    drawings() {
        return new ArgHandler('Sheet.drawings')
            .case(() => {
                return this._drawings;
            })
            .case('string', name => {
                return _.find(this._drawings, ['_name', name]);
            })
            .case(['string', 'blob'], (name, file) => {
                const index = Object.keys(this._drawings).length;
                const id = `rId${index + 1}`;
                const path = `../media/image${index + 1}.png`;

                const relationship = this._drawingRelationships.add("image", path);

                this._workbook._zip.file(
                    path.replace("..", "xl"),
                    file
                );
    ...
    toXmls() {
        ...
        if (this._sheetProtectionNode) {
            xmlq.insertInOrder(node, this._sheetProtectionNode, nodeOrder);
        }
        ...
        if (Object.keys(this._comments).length > 0) {
            ...
            if (!vmlDrawingRelationship) {
                vmlDrawingRelationship = this._relationships.add(
                    "vmlDrawing",
                    `../drawings/vmlDrawing${sheetId}.xml`
                )
            }
        ...
        if (Object.keys(this._drawings).length > 0) {
            ...
            let relationship = this._relationships.findByType("drawing");
            if (!relationship) {
                relationship = this._relationships.add(
                    "drawing",
                    `../drawings/drawing${sheetId}.xml`
                );
            }

            if (!xmlq.hasChild("drawing")) {
                xmlq.removeChild(node, "drawing");
            }
     ...
     * @param {{}} node - The sheet node.
     * @param {{}} [relationshipsNode] - The optional sheet relationships node.
     * @param {{}} drawingsNode - The sheet drawings node.
     * @param {{}} [drawingsReplationshipNode] - The optional sheet drawings relationships node.
     * @returns {undefined}
     * @private
     */
    _init(workbook, idNode, node, relationshipsNode, drawingsNode, drawingsReplationshipNode) {
        if (!node) {
            node = {
                name: "worksheet",
                attributes: {
        ...
        this._drawingRelationships = new Relationships(drawingsReplationshipNode);

        // Delete the optional dimension node
        xmlq.removeChild(this._node, "dimension");

        // Read sheet protection
        this._sheetProtectionNode = xmlq.findChild(this._node, "sheetProtection");
        xmlq.removeChild(this._node, "sheetProtection");
        ...
        // Excel DataValidations

        const extLstNode = xmlq.findChild(this._node, "extLst");
        const excelValidationsNodes = _(extLstNode ? extLstNode.children : [])
            .filter((node) => node.name === "ext")
            .flatMap((node) => node.children)
            .filter((node) => node.name === "x14:dataValidations")
            .flatMap((node) => node.children)
            .filter((node) => node.name === "x14:dataValidation")
            .map((dv) => {
                const sqrefNode = xmlq.findChild(dv, "xm:sqref");
                const sqref = sqrefNode ? sqrefNode.children[0] : null;

                const formula1Node = xmlq.findChild(dv, "x14:formula1");
                const formulaC1 = formula1Node ? formula1Node.children[0] : null;
                const formula1 = formulaC1 ? formulaC1.children[0] : null;

                return sqref && formula1
                    ? {
                          name: "dataValidation",
                          attributes: { ...dv.attributes, sqref },
                          children: [{ name: "formula1", attributes: {}, children: [formula1] }],
                      }
                    : null;
            })
            .compact()
        ...
        excelValidationsNodes.forEach((excelValidationNode) => {
            this._dataValidations[excelValidationNode.attributes.sqref] = excelValidationNode;
        });

        // Create the Drawings.
        if (drawingsNode && drawingsNode.children) {
            this._drawingNodes = drawingsNode.children;
            this._drawingNodes.forEach((drawingNode, i) => {
                if (drawingNode && drawingsReplationshipNode) {
                    if (drawingNode.name.includes('twoCellAnchor')) {
                        const id = drawingNode.children[2].children[1].children[0].attributes['r:embed'];
                        const relationship = this._drawingRelationships.findById(id);
                        if (relationship) {
