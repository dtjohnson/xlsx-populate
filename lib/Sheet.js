...
class Sheet {
     ...
     */ range() {
        return new ArgHandler("Sheet.range")
            ...
            .case(
                ["number", "*", "number", "*"],
                (startRowNumber, startColumnNameOrNumber, endRowNumber, endColumnNameOrNumber) => {
                    return this.range(
     ...
     */ drawings() {
        return new ArgHandler("Sheet.drawings")
            ...
            .case(["string", "blob"], (name, file) => {
                ...
                this._workbook._zip.file(path.replace("..", "xl"), file);
        ...
        return new ArgHandler("Sheet.pageMargins")
            .case(["string"], (attributeName) => {
                ...
                if (attributeValue !== undefined) {
                    ...
                    return parseFloat(
                        this._pageMarginsPresets[this._pageMarginsPresetName][attributeName]
                    );
                } else {
                    return undefined;
                }
        ...
        return new ArgHandler("Sheet.pageMarginsPreset")
            ...
            .case(["string", "object"], (presetName, presetAttributes) => {
                ...
                if (isValidPresetAttributeKeys === false) {
                    throw new Error(
                        `Sheet.pageMarginsPreset: Invalid preset attributes for one or key(s)! - "${Object.keys(
                            presetAttributes
                        )}"`
                    );
                }
            ...
            this._drawingNodes.forEach((drawingNode, i) => {
                if (drawingNode && drawingsReplationshipNode) {
                    if (drawingNode.name.includes("twoCellAnchor")) {
                        const id =
                            drawingNode.children[2].children[1].children[0].attributes["r:embed"];
                        const relationship = this._drawingRelationships.findById(id);
                        if (relationship) {
                            this._drawings[i] = new Drawing(
                                i,
                                workbook,
                                this,
                                drawingNode,
