...
class Sheet {
     ...
     * @ignore
     *//**
     * Set the hyperlink attached to the cell with the given address and options.
     * @param {string} address - The address of the hyperlinked cell.
     * @param {{}}} opts - Options.
     * @param {string|Cell} [opts.hyperlink] - The hyperlink to set, if an instance of Cell the hyperlink is the internal address.
     * @param {string} [opts.tooltip] - Additional text to help the user understand more about the hyperlink.
     * @param {string} [opts.email] - Email address, if set then the hyperlink proprties is ignored.
     * @param {string} [opts.emailSubject] - Email subject.
     * @returns {Sheet} The sheet.
     * @ignore
     */
    hyperlink() {
        return new ArgHandler('Sheet.hyperlink')
            ...
            .case(['string', 'string'], (address, hyperlink) => {
                ...
                return this.hyperlink(address, opts);
            })
            .case(['string', 'object'], (address, opts) => {
                let isHyperlinkInternalAddress = false;
                let hyperlink = opts.hyperlink;
                if (hyperlink instanceof Cell) {
                    isHyperlinkInternalAddress = true;
                    hyperlink = hyperlink.address({ includeSheetName: true });
                } else if (typeof hyperlink === 'string') {
                    isHyperlinkInternalAddress = addressConverter.fromAddress(hyperlink);
                }
                if (opts.email) {
                    const email = opts.email;
                    const subject = opts.emailSubject || '';
                    hyperlink = encodeURI(`mailto:${email}?subject=${subject}`);
                }
                let hyperlinkAttributes;
                ...
                if (opts.tooltip) {
                    hyperlinkAttributes.tooltip = opts.tooltip;
                }
