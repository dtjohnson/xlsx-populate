...
const ColumnRange = require("./ColumnRange");
...
*/
	...
	columnRange() {
		return new ArgHandler('Sheet.columnRange')
			.case('string', address => {
				const ref = addressConverter.fromAddress(address);
				if (ref.type !== 'columnRange') throw new Error('Sheet.columnRange: Invalid address');
				return this.columnRange(ref.startColumnNumber, ref.endColumnNumber);
			})
			.case(['*', '*'], (startColumn, endColumn) => {
				if (!(startColumn instanceof Column)) startColumn = this.column(startColumn);
				if (!(endColumn instanceof Column)) endColumn = this.column(endColumn);
...
*//**
	...
	rowRange() {
		return new ArgHandler('Sheet.rowRange')
			.case('string', address => {
				const ref = addressConverter.fromAddress(address);
				if (ref.type !== 'rowRange') throw new Error('Sheet.rowRange: Invalid address');
				return this.rowRange(ref.startRowNumber, ref.endRowNumber);
			})
			.case(['*', '*'], (startRow, endRow) => {
				if (!(startRow instanceof Row)) startRow = this.row(startRow);
				if (!(endRow instanceof Row)) endRow = this.row(endRow);
   ...
   */
				...
				if (typeof obj === 'string') {
					...
					this._dataValidations[address] = {
						...
						attributes: {
							...
							allowBlank: obj.allowBlank ? obj.allowBlank : false,
							showInputMessage: obj.showInputMessage ? obj.showInputMessage : false,
							prompt: obj.prompt ? obj.prompt : '',
							promptTitle: obj.promptTitle ? obj.promptTitle : '',
							showErrorMessage: obj.showErrorMessage ? obj.showErrorMessage : false,
							error: obj.error ? obj.error : '',
							errorTitle: obj.errorTitle ? obj.errorTitle : '',
	...
	_init(workbook, idNode, node, relationshipsNode, drawingsNode, drawingsReplationshipNode) {
		...
		if (drawingsNode) {
			...
			if (this._drawingNodes) {
				this._drawingNodes.forEach((drawingNode, i) => {
					if (drawingNode && drawingsReplationshipNode) {
						//only handles Icons and Pictures, easy to add Charts and Shapes here as well.
						if(drawingNode.name.includes('twoCellAnchor')) {
							let relationship = this._drawingRelationships.findById(drawingNode.children[2].children[1].children[0].attributes['r:embed']);
							if(relationship) {
								this._drawings[i] = new Drawing(i, workbook, this, drawingNode, relationship);
							}
						} else {
							//console.log('xlsxPopulate: Unsupported Node: ', drawingNode)
						}
					}
				});
			}
