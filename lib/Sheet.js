...
class Sheet {
    ...
    toXmls() {
        ...
        if (this._printOptionsNode.children.length || Object.keys(this._printOptionsNode.attributes).length) {
            xmlq.insertInOrder(node, this._printOptionsNode, nodeOrder);
        }

        // Add the pageMargins if needed.
        if (this._pageMarginsNode.children.length || Object.keys(this._pageMarginsNode.attributes).length) {
            xmlq.insertInOrder(node, this._pageMarginsNode, nodeOrder);
        }
     ...
     * @param {undefined|boolean} attributeEnabled - If `undefined` then the attribute is removed, otherwise set as a boolean value to enable/disable the print option.
     * @returns {Sheet} The sheet.
     */
    printOptions() {
        ...
        return new ArgHandler('Sheet.printOptions')
            ...
            .case(['string', 'boolean'], (attributeName, attributeEnabled) => {
                checkAttributeName(attributeName);
                this._printOptionsNode.attributes[attributeName] = attributeEnabled ? 1 : 0;
                return this;
            })
            .handle(arguments);
    }
     ...
     * @param {undefined|boolean} enabled - If `undefined` then attribute is removed, otherwise set as a boolean value to enable/disable the gridLines.
     * @returns {Sheet} The sheet.
     */
    printGridLines() {
     ...
     * @param {undefined|number|string} attributeStringValue - If the value is `undefined` then the attribute is removed, otherwise set the associated attribute value
     * @returns {Sheet} The sheet.
     */
    pageMargins() {
        const supportedAttributeNames = [
        ...
        return new ArgHandler('Sheet.pageMargins')
            .case(['string'], attributeName => {
                checkAttributeName(attributeName);
                const value = this._pageMarginsNode.attributes[attributeName];
     ...
     * @property {number} xSplit=0 (Horizontal Split Position) Horizontal position of the split, in 1/20th of a point;
     * 0 (zero) if none. If the pane is frozen, this value indicates the number of columns visible in the top pane.
     * @property {number} ySplit=0 (Vertical Split Position) Vertical position of the split, in 1/20th of a point; 0
    ...
    pane() {
        ...
        return new ArgHandler('Sheet.pane')
            .case(() => {
                return paneNode ? undefined : paneNode.attributes;
            })
            .case(['nil'], () => {
                delete sheetViewNode.children.pane;
                return this;
            })
            .case(['object'], paneAttributes => {
                const attributes = Object.assign({ xSplit: 0, ySplit: 0, activePane: 'bottomRight' }, paneAttributes);
     ...
     * @param {number} xSplit the number of columns visible in the top pane.
     * @param {number} ySplit the number of rows visible in the left pane.
     * @return {Sheet} The sheet
     *//**
     * freezes Panes for this sheet.
    ...
    feezePanes() {
        return new ArgHandler('Sheet.feezePanes')
            .case(['int', 'int'], (xSplit, ySplit) => {
                const topLeftCell = addressConverter.columnNumberToName(xSplit + 1) + (ySplit + 1);
                return this.pane({ state: 'frozen', topLeftCell, xSplit, ySplit });
            })
            .case(['string'], topLeftCell => {
                const ref = addressConverter.fromAddress(topLeftCell);
                return this.pane({ state: 'frozen', topLeftCell, xSplit: ref.columnNumber - 1, ySplit: ref.rowNumber - 1 });
            })
            .handle(arguments);
    }
    ...
    splitPanes(xSplit, ySplit) {
        const topLeftCell = undefined;
        return this.pane({ state: 'split', topLeftCell, xSplit, ySplit });
    }
