				...
				this._drawingNodes.forEach((drawingNode, i) => {
					if (drawingNode && drawingsReplationshipNode) {
						...
						if(drawingNode.name.includes('twoCellAnchor')) {
							let relationship = new Relationships(drawingsReplationshipNode)
							relationship = relationship.findById(drawingNode.children[2].children[1].children[0].attributes['r:embed']);
							if(relationship) {
								this._drawings[i] = new Drawing(workbook, this, drawingNode, relationship);
								relationship = null;
							}
						} else {
