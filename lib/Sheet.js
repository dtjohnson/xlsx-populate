...
class Sheet {
    ...
    active() {
        return new ArgHandler('Sheet.active')
            ...
            .case('boolean', active => {
                if (!active) throw new Error("Sheet.active: Deactivating sheet directly not supported. Activate a different sheet instead.");
    ...
    activeCell() {
        debug("activeCell(%o)", arguments);
        const sheetViewsNode = xmlq.findChild(this._node, "sheetViews");
        const sheetViewNode = xmlq.findChild(sheetViewsNode, "sheetView");
    ...
    column(columnNameOrNumber) {
        ...
        // Find existing col node.
        const existingColNode = _.find(this._colsNode.children, colNode => {
        ...
        if (existingColNode) {
            ...
            if (columnNumber > existingColNode.attributes.min) {
                const beforeColNode = _.cloneDeep(existingColNode);
                beforeColNode.attributes.max = columnNumber - 1;
                xmlq.insertBefore(this._colsNode, beforeColNode, existingColNode);
            }

            xmlq.insertBefore(this._colsNode, colNode, existingColNode);

            if (columnNumber < existingColNode.attributes.max) {
                const afterColNode = _.cloneDeep(existingColNode);
                afterColNode.attributes.min = columnNumber + 1;
                xmlq.insertBefore(this._colsNode, afterColNode, existingColNode);
            }

            xmlq.removeChild(this._colsNode, existingColNode);
        } else {
            colNode = {
                name: 'col',
                attributes: {
            ...
            xmlq.appendChild(this._colsNode, colNode);
        }
    ...
    // TODO: Make sure to inactivate if hidden
    /**
    ...
    move(before) {
        this.workbook().moveSheet(this, before);
        return this;
    }
    ...
    selection() {
        debug("selections(%o)", arguments);
        const sheetViewsNode = xmlq.findChild(this._node, "sheetViews");
        const sheetViewNode = xmlq.findChild(sheetViewsNode, "sheetView");
        let selectionNode = xmlq.findChild(sheetViewNode, "selection");
        return new ArgHandler('Sheet.selects')
            .case(() => {
                const sqref = selectionNode ? selectionNode.attributes.sqref : "A1";
                const addresses = sqref.split(" ");
                return _.map(addresses, address => {
                    return address.indexOf(":") >= 0 ? this.range(address) : this.cell(address);
                });
            })
            .case("array", selections => {
                const sqref = _.map(selections, selection => {
    ...
    existingColumnStyleId(columnNumber) {
        ...
        const colNode = _.find(this._colsNode.children, colNode => {
            const min = colNode.attributes.min;
            const max = colNode.attributes.max;
            return columnNumber >= min && columnNumber <= max;
        });
