...
module.exports = {
    ...
    _query(node, query, result, fullResult) {
        debug("_query(%s, %o, %o, %o)", node, query, result, fullResult);
        ...
        for (const key in query) {
            ...
            if (key === "#text") {
                const textValue = utils.getNodeType(node.firstChild) === "text" ? node.textContent : null;
                ...
                for (let i = 0; i < node.childNodes.length; i++) {
                    ...
                    if (childNode.tagName !== key) continue;
                    if (queryValue === null) return null;
