...
class _Style {
    ...
    _getColor(node, path) {
        ...
        if (jq.has($, "indexed")) color.rgb = colorIndexes[$.indexed];
    ...
    _setColor(node, path, color) {
        ...
        if (path) path = `${path}.`;
        return jq.set(node, {
            [`${path}rgb`]: color && color.rgb && color.rgb.toUpperCase(),
            [`${path}indexed`]: null,
            [`${path}theme`]: color && color.theme,
            [`${path}tint`]: color && color.tint
        });
    }
    ...
    _get_fill() {
        ...
        if (patternType) {
            return {
                type: "pattern",
                pattern: patternType,
                foreground: this._getColor(patternFill, "fgColor[0].$"),
                background: this._getColor(patternFill, "bgColor[0].$")
            };
        }
    ...
    _set_fill(fill) {
        ...
        if (fill.type === "pattern") {
            ...
            this._setColor(patternFill, "fgColor[0].$", fill.foreground);
            this._setColor(patternFill, "bgColor[0].$", fill.background);
            jq.set(patternFill, {
        ...
        if (fill.type === "gradient") {
            ...
            _.forEach(fill.stops, (fillStop, i) => {
                ...
                jq.set(stop, '$.position', fillStop.position);
            });
        ...
        else if (jq.has(fill, "rgb") || jq.has(fill, "theme")) fill = { color: fill };
    ...
    _getBorder() {
        ...
        ["left", "right", "top", "bottom", "diagonal"].forEach(side => {
            ...
            jq.set(result, `${side}.color`, this._getColor(this._borderNode, `${side}[0].color[0].$`));
            if (side === "diagonal") {
                const up = jq.get(this._borderNode, "$.diagonalUp");
                const down = jq.get(this._borderNode, "$.diagonalDown");
                let direction;
    ...
    _setBorder(settings) {
        _.forOwn(settings, (setting, side) => {
            ...
            if (setting.hasOwnProperty("color")) {
                this._setColor(this._borderNode, `${side}[0].color[0].$`, setting.color);
            }
            ...
            if (jq.isEmpty(this._borderNode, `${side}[0].color[0].$`)) delete this._borderNode[side][0].color;
            if (jq.isEmpty(this._borderNode, `${side}[0].$`)) delete this._borderNode[side][0].$;
        });

        if (jq.isEmpty(this._borderNode, '$')) delete this._borderNode.$;
    }
