...
class Cell {
    ...
    find(pattern, replacement) {
        ...
        let value = this.dataValidation();
        if (typeof value === 'object') {
            if (_.isNil(replacement)) {
                return pattern.test(value.formula1);
            } else {
                let replaced = value.formula1.replace(pattern, replacement);
                if (replaced !== value.formula1)
                    this.dataValidation(value);
                //dont return true as we need to check the cells value as well just incase its already set
            }
        }
        ...
        if (_.isNil(replacement)) {
            ...
            replaced = value.replace(pattern, replacement);
