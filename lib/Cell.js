...
class Cell {
     ...
     */
    clear() {
        const sharedFormulaId = this._sharedFormulaId;

        delete this._value;
        delete this._formula;
        delete this._sharedFormulaId;
        delete this._sharedFormulaRef;
    ...
    value() {
        return new ArgHandler('Cell.value')
            .case(() => {
                ...
                // } else if (type === "e") {
                //     const error = xmlq.findChild(this._node, 'v').children[0];
                //     value = FormulaError.getError(error);
                // } else {
    ...
    getSharedRefFormula() {
        ...
        return this._sharedFormulaRef && this._formula;
    }
     ...
     */
    sharesFormula(id) {
        return this._sharedFormulaId === id;
        // const fNode = xmlq.findChild(this._node, 'f');
    ...
    setSharedFormula(id, formula, sharedRef) {
        ...
        this._formula = formula;
        this._sharedFormulaRef = sharedRef;
    }
    ...
    toXml() {
        const node = {
            name: 'c',
            attributes: this._remainingAttibutes || {},
            children: []
        };
        ...
        if (!_.isNil(this._sharedFormulaId)) {
            const fNode = { name: 'f', attributes: { t: 'shared', si: this._sharedFormulaId } };
            if (!_.isNil(this._formula)) fNode.children = [this._formula];
            if (!_.isNil(this._sharedFormulaRef)) fNode.attributes.ref = this._sharedFormulaRef;
            node.children.push(fNode);
        } else if (!_.isNil(this._formula)) {
            const fNode = { name: 'f', children: [this._formula] };
            node.children.push(fNode);
        } else if (!_.isNil(this._value)) { // Don't emit value if a formula is set
            let type, text;
            if (typeof this._value === "string" || _.isObject(this._value)) { // TODO: Rich text is array for now
                type = "s";
    ...
    _parseNode(node) {
        ...
        if (fNode) {
            ...
            this._sharedFormulaRef = fNode.attributes.ref;
            this._formula = fNode.children[0];

            if (this._sharedFormulaRef) {
        ...
        if (type === "s") {
            ...
            this._value = xmlq.findChild(xmlq.findChild(node, 'is'), 't').children[0];
        } else if (type === "b") {
        ...
        if (!_.isEmpty(node.attributes)) this._remainingAttibutes = node.attributes;
