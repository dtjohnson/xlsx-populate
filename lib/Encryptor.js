...
const CFB_FILE_SIGNATURE = Buffer.from([0xd0, 0xcf, 0x11, 0xe0, 0xa1, 0xb1, 0x1a, 0xe1]);
...
class Encryptor {
    ...
    decryptAsync(data, password) {
        ...
        const encryptionInfoBuffer = _.find(parsed.FileIndex, { name: "EncryptionInfo" }).content;
        const encryptedPackageBuffer = _.find(parsed.FileIndex, { name: "EncryptedPackage" }).content;
    ...
    isEncrypted(data) {
        const header = data.slice(0, CFB_FILE_SIGNATURE.length);
    }
    ...
    _parseEncryptionInfoAsync(buffer) {
        ...
        const xml = buffer.slice(ENCRYPTION_INFO_PREFIX.length).toString();
...
// const i = fs.readFileSync("../examples/encryption/input.xlsx");
//
// encryptor.decryptAsync(i, "open sesame")
//     .then(b => {
//         fs.writeFileSync("../examples/encryption/input.zip", b)
//         const enc = encryptor.encrypt(b, "foo")
//         fs.writeFileSync("../examples/encryption/o.xlsx", enc)
//     });

// const o = fs.readFileSync("../examples/encryption/output.xlsx");
//
// Promise.all([
//     encryptor.decryptAsync(i, "open sesame").then(b => fs.writeFileSync("../examples/encryption/input.zip", b)),
//     encryptor.decryptAsync(o, "open sesame").then(b => fs.writeFileSync("../examples/encryption/output.zip", b))
