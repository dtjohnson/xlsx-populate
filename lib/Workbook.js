...
class Workbook {
    ...
    _initAsync(data) {
        ...
        return JSZip.loadAsync(data)
            ...
            .then(nodes => {
                ...
                const drawingsNode = nodes[5];
                const drawingsRelationshipsNode = nodes[6];
                ...
                this._drawingsNode = drawingsNode;
                this._drawingsRelationshipsNode = drawingsRelationshipsNode;
                this._drawings;
                ...
                externals.Promise.all(_.map(this._sheetsNode.children, (sheetIdNode, i) => {
                ...
                //load drawings into array
                externals.Promise.all(_.map(this._drawingsNode.children, (drawingIdNode, i) => {
                    let name = xmlq.findChild(xmlq.findChild(xmlq.findChild(drawingIdNode, "xdr:pic"), "xdr:nvPicPr"), "xdr:cNvPr").attributes.name;
                    let id = xmlq.findChild(xmlq.findChild(xmlq.findChild(drawingIdNode, "xdr:pic"), "xdr:blipFill"), "a:blip").attributes['r:embed'];
                    this._drawings[id] = {
                        'name': name,
                        'id': id,
                        'filePath': null
                ...
                //add the file paths to drawings..
                return externals.Promise.all(_.map(this._drawingsRelationshipsNode.children, (relationshipNode, i) => {
                    this._drawings[relationshipNode.attributes.Id].filePath = relationshipNode.attributes.Target;
                }));
            })
    ...
    swapImage(name, imageData) {
        for(let d in this._drawings) {
            if(this._drawings[d].name == name) {
                this._zip.files[this._drawings[d].filePath.replace('..', 'xl')]._data = imageData;
            }
        }
    }
