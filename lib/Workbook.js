...
class Workbook {
    ...
    /**
     * Gets the 2D Array of the sheet with the provided name or index (0-based).
     * @param {string|number} sheetNameOrIndex - The sheet name or index.
     * @param {{}} config - The config of this function.
     * @returns {Array.<Array.<*>>}|undefined} The 2D Array of the sheet or undefined if not found.
     */
    sheet2DArray(sheetNameOrIndex,config){
        config=config||{
          normalized:false
        }
        const cells=this.sheet(sheetNameOrIndex).usedRange().cells()
      
        const array2nd=[]
        function value(cell){
          const rawvalue=cell.value()
          if(!rawvalue)return
          return rawvalue.text ? rawvalue.text() : rawvalue
        }
        for (let rindex = 0; rindex < cells.length-1; rindex++) {
          const array1st=[]
          for (let cindex = 0; cindex < cells[0].length-1; cindex++) {
            array1st.push(value(cells[rindex][cindex]))
          }
          array2nd.push(array1st)
        }
        if(config.normalized){
          for (const range in sheet._mergeCells) {
            const [startRange,endRange]=range.split(':')
            const [startAddress,endAddress]=[startRange,endRange].map(range=>{
              return {
                row:(Number(range.match(/[0-9]+$/)[0])-1)  ,
                column:(range.match(/^[A-Z]+/)[0].split('')
                  .reduce((a,b)=>a*26+b.charCodeAt()-'A'.charCodeAt()+1,0)-1)
              }
            })
            for (let rindex = startAddress.row; rindex < endAddress.row+1; rindex++) {
              for (let cindex = startAddress.column; cindex < endAddress.column+1; cindex++) {
                array2nd[rindex][cindex]=value(cells[startAddress.row][startAddress.column])
              }
            }      
          }
        }
