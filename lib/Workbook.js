...
class Workbook {
    ...
    outputAsync(opts) {
        ...
        this._sheets.forEach((sheet, i) => {
            ...
            let tmp;
            _.each(sheet._drawings, (drawing, i) => {
                tmp = drawing.toXml();
                sheet._drawingNodes[drawing._nodeId] = tmp[0];
                sheet._drawingRelationships[sheet._drawingRelationships.getIndexById(drawing._id)] = tmp[1];
            });
            ...
            drawingRelationshipXml = xmlBuilder.build(sheet._drawingRelationships);

            this._zip.file(drawingPath, drawingsXml, zipFileOpts);
            this._zip.file(drawingRelsPath, drawingRelationshipXml, zipFileOpts);
