...
class Workbook {
    ...
    _select(ref) {
        if (ref.type === 'column') {
            return this.sheet(ref.sheetName).column(ref.columnNumber);
        }

        if (ref.type === 'row') {
            return this.sheet(ref.sheetName).row(ref.rowNumber);
        }

        if (ref.type === 'cell') {
            return this.sheet(ref.sheetName).cell(ref.rowNumber, ref.columnNumber);
        }

        if (ref.type === 'range') {
            return this.sheet(ref.sheetName).range(ref.startRowNumber, ref.startColumnNumber, ref.endRowNumber, ref.endColumnNumber);
        }

        if (ref.type === 'group') {
            const items = ref.refs.map(ref => this._select(ref));
