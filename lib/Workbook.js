...
const xml2jsOpts = { attrValueProcessors: [xml2js.processors.parseNumbers] };
...
class Workbook {
    ...
    _initializeAsync(data) {
        ...
        return JSZip.loadAsync(data)
            .then(zip => {
                ...
                return Promise.all([
                    this._zip.file("[Content_Types].xml").async("string")
                        .then(text => xml2js.parseStringAsync(text, xml2jsOpts)),
            ...
            .spread((contentTypesNode, relationshipsNode, sharedStringsNode, styleSheetNode, workbookText) => {
    ...
    outputAsync(type) {
        ...
        this._zip.file("[Content_Types].xml", builder.buildObject(this._contentTypes.toObject()), fileOpts);
        this._zip.file("xl/_rels/workbook.xml.rels", builder.buildObject(this._relationships.toObject()), fileOpts);
        this._zip.file("xl/sharedStrings.xml", builder.buildObject(this._sharedStrings.toObject()), fileOpts);
        this._zip.file("xl/styles.xml", builder.buildObject(this._styleSheet.toObject()), fileOpts);
        this._zip.file("xl/workbook.xml", this._xml.toString(), fileOpts);
// console.log(builder.buildObject(this._styleSheet.toObject()));
        this._sheets.forEach((sheet, i) => {
            this._zip.file(`xl/worksheets/sheet${i + 1}.xml`, sheet.toString(), fileOpts);
        });
