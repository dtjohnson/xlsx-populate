...
class Workbook {
    ...
    definedName() {
        return new ArgHandler("Workbook.definedName")
            .case('string', name => {
                return this.scopedDefinedName(null, name);
            })
            .case(['string', '*'], (name, refersTo) => {
                this.scopedDefinedName(null, name, refersTo);
                return this;
            })
            .handle(arguments);
    }
     ...
     * @param {Sheet} sheetScope - The sheet the name is scoped to. Use null for workbook scope.
     * @param {string} name - The defined name.
     ...
     * @ignore
     */
// TODO: When adding/moving sheets the scoped defined names are messed up
    scopedDefinedName(sheetScope, name, refersTo) {
        ...
        let definedNameNode = definedNamesNode && _.find(definedNamesNode.children, node => node.attributes.name === name && node.attributes.localSheetId === localSheetId);
        ...
        return new ArgHandler('Workbook.scopedDefinedName')
            ...
            .case(['*', 'string', '*'], () => {
                ...
                if (!definedNameNode) {
                    ...
                    if (_.isInteger(localSheetId)) definedNameNode.attributes.localSheetId = localSheetId;
