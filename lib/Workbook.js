...
// Initialize the encryptor, parser, and builder.
...
class Workbook {
    ...
    outputAsync(opts) {
        ...
        this._sheets.forEach((sheet, i) => {
            ...
            //drawings for this sheet path
            const drawingPath = `xl/drawings/drawing${i + 1}.xml`;
            const drawingRelsPath = `xl/drawings/_rels/drawing${i + 1}.xml.rels`;

            let drawingsXml = [];
            let drawingRelationshipXml = [];
            let tmp;
            let drawings = sheet._drawings;
            _.each(drawings, (drawing, i) => {
                tmp = drawing.toXml();
                drawingsXml.push(tmp[0]);
                drawingRelationshipXml.push(tmp[1]);
            });

            drawingsXml = xmlBuilder.build({
                name: 'xdr:wsDr',
                attributes: {
                    'xmlns:xdr': 'http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing',
                    'xmlns:a': 'http://schemas.openxmlformats.org/drawingml/2006/main'
                },
                children: drawingsXml
            });

            drawingRelationshipXml = xmlBuilder.build({
                name: 'Relationships',
                attributes: {
                    'xmlns': 'http://schemas.openxmlformats.org/package/2006/relationships',
                },
                children: drawingRelationshipXml
            })

            if(drawingsXml.children) this._zip.file(drawingPath, drawingsXml, zipFileOpts);
            if(drawingRelationshipXml.children) this._zip.file(drawingRelsPath, drawingRelationshipXml, zipFileOpts);
    ...
    _initAsync(data, opts) {
        ...
        data = this._convertInputToBuffer(data, opts.base64);
        ...
        return Promise.resolve()
            ...
            .then(nodes => {
                ...
                return externals.Promise.all(_.map(this._sheetsNode.children, (sheetIdNode, i) => {
                    ...
                    //load sheets
                    return this._parseNodesAsync([`xl/worksheets/sheet${i + 1}.xml`, `xl/worksheets/_rels/sheet${i + 1}.xml.rels`, `xl/drawings/drawing${i + 1}.xml`, `xl/drawings/_rels/drawing${i + 1}.xml.rels`]).then(nodes => {
     ...
     * @private
     */
    _convertInputToBuffer(input, base64) {
        if (Buffer.isBuffer(input)) return input;
        if (process.browser && input instanceof Blob) return Buffer.from(input, 'binary');
