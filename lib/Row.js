...
class Row {
     ...
     * @returns {undefined|number|Row} The height (or undefined) if getting, the row if setting.
     */
    height() {
        debug('height(%o)', arguments);
        return new ArgParser('Row.height')
            .case(() => {
     ...
     * @returns {boolean|Row} A flag indicating whether the row is hidden if getting, the row if setting.
     */
    hidden() {
        debug('hidden(%o)', arguments);
        return new ArgParser("Row.hidden")
            .case(() => {
