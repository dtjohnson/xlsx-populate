...
class Workbook {
    ...
    _initializeAsync(data) {
        return JSZip.loadAsync(data)
            ...
            .spread((workbookText, relsText, sharedStringsText) => {
                console.log(parser.parseFromString(workbookText).toString());
    ...
    createSheet(sheetName, index) {
        ...
        for (var i = this._relsXML.childNodes.length - 1; i >= 0; i--) {
            ...
            } else if (rnode.getAttribute("Type") === "http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings") {
        ...
        if (true || !hasSharedStrings) {
            const relNode = parser.parseFromString('<Relationship Id="rId4" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings" Target="sharedStrings.xml"/>');
            this._relsXML.appendChild(relNode);
        }
