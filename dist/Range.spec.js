...
describe("Range", () => {
    ...
    describe("forEach", () => {
        it("should call the callback for each cell", () => {
            ...
            expect(callback.calls.argsFor(0)).toEqual(["CELL[3, 2]", 0, 0, range]);
            expect(callback.calls.argsFor(1)).toEqual(["CELL[3, 3]", 0, 1, range]);
            expect(callback.calls.argsFor(2)).toEqual(["CELL[4, 2]", 1, 0, range]);
            expect(callback.calls.argsFor(3)).toEqual(["CELL[4, 3]", 1, 1, range]);
            expect(callback.calls.argsFor(4)).toEqual(["CELL[5, 2]", 2, 0, range]);
            expect(callback.calls.argsFor(5)).toEqual(["CELL[5, 3]", 2, 1, range]);
        });
    });
    describe("formula", () => {
    ...
    describe("reduce", () => {
        it("should call the callback for each cell and return the aggregate value", () => {
            ...
            expect(callback.calls.argsFor(0)).toEqual(["INITIAL", "CELL[3, 2]", 0, 0, range]);
            expect(callback.calls.argsFor(1)).toEqual(["INITIAL RETURN[0, 0]", "CELL[3, 3]", 0, 1, range]);
            expect(callback.calls.argsFor(2)).toEqual(["INITIAL RETURN[0, 0] RETURN[0, 1]", "CELL[4, 2]", 1, 0, range]);
            expect(callback.calls.argsFor(3)).toEqual(["INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0]", "CELL[4, 3]", 1, 1, range]);
            expect(callback.calls.argsFor(4)).toEqual(["INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0] RETURN[1, 1]", "CELL[5, 2]", 2, 0, range]);
            expect(callback.calls.argsFor(5)).toEqual(["INITIAL RETURN[0, 0] RETURN[0, 1] RETURN[1, 0] RETURN[1, 1] RETURN[2, 0]", "CELL[5, 3]", 2, 1, range]);
        });
    });
    describe("sheet", () => {
