...
Workbook.fromBlankAsync("./row col size.xlsx")
    .then(workbook => {
       ...
       console.log(JSON.stringify(sheet.cell("A10").value("comic sans").style("fontFamily", "Comic Sans MS").style("fontFamily")));
        // console.log(JSON.stringify(sheet.cell("A11").value("left").style().horizontalAlignment("left").horizontalAlignment()));
        // console.log(JSON.stringify(sheet.cell("A12").value("center").style().horizontalAlignment("center").horizontalAlignment()));
        // console.log(JSON.stringify(sheet.cell("A13").value("right").style().horizontalAlignment("right").horizontalAlignment()));
        // console.log(JSON.stringify(sheet.cell("A14").value("top").style().verticalAlignment("top").verticalAlignment()));
        // console.log(JSON.stringify(sheet.cell("A15").value("middle").style().verticalAlignment("center").verticalAlignment()));
        // console.log(JSON.stringify(sheet.cell("A16").value("bottom").style().verticalAlignment("bottom").verticalAlignment()));
        // console.log(JSON.stringify(sheet.cell("A17").value("this is wrapped text").style().wrappedText(true).wrappedText()));
        ...
        // console.log(JSON.stringify(sheet.cell("A28").value("indent").style().indent(2).indent()));
        // console.log(JSON.stringify(sheet.cell("A29").value("text rotation").style().textRotation(20).textRotation()));
        // console.log(JSON.stringify(sheet.cell("A30").value("angle counterclockwise").style().angleTextCounterclockwise(true).angleTextCounterclockwise()));
        // console.log(JSON.stringify(sheet.cell("A31").value("angle clockwise").style().angleTextClockwise(true).angleTextClockwise()));
        // console.log(JSON.stringify(sheet.cell("A32").value("verticalText").style().verticalText(true).verticalText()));
        // console.log(JSON.stringify(sheet.cell("A33").value("rotate text up").style().rotateTextUp(true).rotateTextUp()));
        // console.log(JSON.stringify(sheet.cell("A34").value("rotate text down").style().rotateTextDown(true).rotateTextDown()));
